...
The cleaned accelerametor dataset contains `r nrow(accel_data)` observations/minutes and `r ncol(accel_data)` variables. Variables include the week, day, hour, and minute. Ultimately, the data represents `r nrow(accel_data) / 1440` days or `r as.integer(nrow(accel_data) / 24)` hours worth of data. The data is highly right skewed due to the presence of outliers; as such, in addition to total activity, the median and/or geometric mean are used throughout this analysis. The median and geometric average activity count throughout the entire observation period were `r median(accel_data$activity_count)` and `r round(exp(mean(log(accel_data$activity_count))), 2)` respectively. The modal activity count was 1, indicating no activity, with `r accel_data %>% group_by(activity_count) %>% summarize(n = n()) %>% filter(n == max(n)) %>% .$n` observed minutes.

### Exploratory analyses

```{r density plots of activity over the weeks, out.width = "80%", fig.align = "center", dpi = 300, warning = FALSE}
