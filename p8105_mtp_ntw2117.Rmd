...
The first steps in this analysis required importing and tidying the accelerametor data. After importing the data I noticed that each row represented one day during a week in the data set and that the variables activity_1 through activity_4 represented minutes of the day. I thus transformed the dataset from wide form to long form by creating a new variable called minute that recorded what minute of the day each row corresponds to. I then created a new variable called hour that recorded what hour of the day (on a 24 hour clock) each observation belonged to. Lastly, I created  another week variable that collapsed the weeks into 9 week intervals. I also ordered the day variable so that the factor levels followed the actual order of the week.

The cleaned accelerametor dataset contains `r nrow(accel_data)` observations/minutes and `r ncol(accel_data)` variables. This represents `r nrow(accel_data) / 1440` days or `r as.integer(nrow(accel_data) / 24)` hours worth of data. The data is highly right skewed; as such, in addition to total activity, the median and/or geometric mean are used throughout this analysis in leu of the mean. The median and geometric average activity count throughout the entire observation period were `r median(accel_data$activity_count)` and `r round(exp(mean(log(accel_data$activity_count))), 2)` respectively. The modal activity count was 1, indicating no activity, with `r accel_data %>% group_by(activity_count) %>% summarize(n = n()) %>% filter(n == max(n)) %>% .$n` observed minutes.

```{r table of median and geo mean according to day of the week}
...
```{r density plots of activity over the weeks, out.width = "75%", fig.align = "center", dpi = 300, warning = FALSE}
...
```{r activity by day line plot, fig.asp = .7, out.width = "70%", dpi = 300, fig.align = "center"}
accel_data %>% 
  group_by(day) %>% 
...
There must be some days in which the individual was very active on Sundays and not very active on Mondays and Tuesdays that we see opposite plots. The geometric mean plot shows a weekly trend in activity while the total activity plot shows the days where the most and least activity occured throughout the whole observation period. 
...
```{r total activity across weeks, out.width = "100%", fig.width = 10, fig.asp = 0.7, fig.align = "center", dpi = 300}
...
wrap_elements(total_actvty / total_actvty_week_day) + 
  ggtitle("Total activity trends over the observation period")
```

```{r plotting the mean and median activity count across weeks by day, warning = FALSE, message = FALSE, fig.align = "center", fig.width = 10, fig.asp = .8, out.width = "100%", dpi = 300}
total_actvty_week_day <- accel_data %>% 
  group_by(week, day) %>% 
  summarize(total_actvty = sum(activity_count)) %>% # using geometric mean due to how skewed data is
  ggplot(aes(x = week, y = total_actvty)) + 
